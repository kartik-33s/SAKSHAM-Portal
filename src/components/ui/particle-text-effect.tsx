...
const DEFAULT_WORDS = ["SAKSHAM", "KNOW RIGHTS", "PROTECT FREEDOM", "CONSTITUTION", "BNSS 2023"];
...
export function ParticleTextEffect({ words = DEFAULT_WORDS, className }: ParticleTextEffectProps) {
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    // Generate new bright/vibrant color
    const newColor = {
  ...
  return (
    <div className={`flex flex-col items-center justify-center bg-black p-4 rounded-[12px] border border-white/10 ${className || ''}`}>
      <canvas
        ref={canvasRef}
        className="border border-zinc-800 rounded-lg shadow-2xl w-full max-w-2xl bg-black"
        style={{ maxWidth: "100%", height: "auto" }}
      />
      <div className="mt-3 text-white text-xs text-center max-w-md">
        <p className="text-zinc-400 text-[11px]">
          Right-click and drag to disperse particles • Cycles keywords automatically every 4 seconds
        </p>
      </div>
    </div>
  );
