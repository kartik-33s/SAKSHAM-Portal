...
/* Motiq design tokens                                                        */
/* -------------------------------------------------------------------------- */
const MOTIQ_TOKENS = "@layer motiq{:root{--motiq-accent:#e60000;--motiq-accent-text:#e60000;--motiq-bg:#f7f9fc;--motiq-border:#dce4ef;--motiq-border-strong:#c5d1e1;--motiq-fg:#101828;--motiq-fg-secondary:#344054;--motiq-muted:#667085;--motiq-secondary-accent:#e60000;--motiq-signature:#e60000;--motiq-surface:#ffffff;--motiq-surface-2:#f8fafd}}@layer motiq{.dark,[data-theme=\"dark\"]{--motiq-accent:#e60000;--motiq-accent-text:#ff4d4d;--motiq-bg:#080c14;--motiq-border:#263449;--motiq-border-strong:#354863;--motiq-fg:#f8fafc;--motiq-fg-secondary:#cbd5e1;--motiq-muted:#9caabd;--motiq-secondary-accent:#ff3333;--motiq-signature:#e60000;--motiq-surface:#111827;--motiq-surface-2:#192337}}";
...
function ringGradient(beams: 1 | 2, colors: [string, string?] | undefined): string {
  const tail0 = colors?.[0] ?? "var(--motiq-accent, #e60000)";
  const head0 = colors?.[0] ?? "var(--motiq-secondary-accent, #ff4444)";
  const stops = [
    "transparent 0deg",
    comet(tail0, head0, `color-mix(in srgb, ${head0} 22%, #ffffff)`, 55, 0),
  ];
  if (beams === 2) {
    const c1 = colors?.[1] ?? "var(--motiq-signature, #ffffff)";
...
@media (forced-colors: active) {
  ...
  return (
    <div
      ...
      onBlur={settle}
      className={cn(
        "relative w-full border border-[#25282b]/60 bg-[#25282b] p-6",
        cls,
        className,
      )}
      style={{
